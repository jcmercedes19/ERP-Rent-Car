...
export const generateNCF = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
...
export const generatePreCheckInLink = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError("unauthenticated", "Debes iniciar sesión.");
  }
...
export const paymentWebhook = functions.https.onRequest(async (req, res) => {
  ...
  try {
    const intentRef = db.collection("paymentIntents").doc(intentId as string);
    ...
    if (status === 'approved') {
      ...
      await intentRef.update({ 
        status: status, // 'failed' o 'cancelled'
        updatedAt: admin.firestore.FieldValue.serverTimestamp() 
      });
